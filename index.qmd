...
![](images/NR_kinematics.png)
![](images/NR_kinetics.png)
...
![](images/HH_kinematics.png)
![](images/HH_kinetics.png)
...
![](images/HS_kinematics.png)
![](images/HS_kinetics.png)
...
![](images/TEMR_kinematics.png)
![](images/TEMR_kinetics.png)

#### TEMR and Hydrogel (TEMR+HH)

![](images/TEMR_HH_kinematics.png)
![](images/TEMR_HH_kinetics.png)

#### TEMR and Keratin Gel (TEMR+KG)

![](images/TEMR_KG_kinematics.png)
![](images/TEMR_KG_kinetics.png)
...
The analysis pipeline reads from Parquet files generated by the movedb-core ingestion pipeline. The following code demonstrates how to aggregate and compare groups using the `rat_vml.analysis.aggregation` module.
...
# Load IK results for week24
results = load_results("data/processed", result_type="ik", session="week24")

# Aggregate by group
control = aggregate_by_group(results, group="control", session="week24")
nr = aggregate_by_group(results, group="nr", session="week24")

print(f"Control: {control.n_subjects} subjects")
print(f"NR: {nr.n_subjects} subjects")
```

### Comparing Groups

```python
# Compare No Repair vs Control
comparison = compare_groups(nr, control)

# Access SPM t-test results per coordinate
for coord, spm_result in comparison.spm_results.items():
    if spm_result is not None:
        print(f"{coord}: {spm_result}")
```

### Generating Figures

```python
from scripts.plot_helpers import plot_group_kinematics, plot_group_comparison

# Plot kinematics for NR vs Control
fig = plot_group_kinematics(
    data_dir="data/processed",
    group="nr",
    session="week24",
    output_path="figures/NR_kinematics.png",
)
...
fig = plot_group_comparison(
    ...
    session="week24",
    result_type="ik",
    output_path="figures/NR_vs_Control_spm.png",
)
```
...
```python
# Generate figures for all treatment groups
groups = ["nr", "temr", "hh", "hs", "temr_hh", "temr_kg"]
...
for group in groups:
    fig = plot_group_kinematics(
        ...
        group=group,
        session="week24",
        output_path=f"figures/{group}_kinematics.png",
    )
    plt.close(fig)
```
