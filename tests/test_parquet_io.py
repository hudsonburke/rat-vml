...
def test_parquet_to_trc(sample_markers, tmp_path):
    ...
    # osimpy TRC format: metadata lines + tab-separated data
    assert "Walk01.trc" in content
    assert "r_asis" in content
    assert "NumFrames" in content
...
def test_parquet_to_fp_mot(sample_forceplates, tmp_path):
    ...
    # osimpy MOT format: header + tab-separated data
...
def test_ext_loads_xml_content(sample_forceplates, tmp_path):
    ...
    assert "ExternalLoads" in content or "FP1" in content
